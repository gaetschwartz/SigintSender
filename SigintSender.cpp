...
void sendCtrlC(int pid)
{
    ...
    if (AttachConsole(pid))
    {
        ...
        printf("Failed to send CTRL+C to pid %d", pid); // failure, PID might no longer exist, show a GUI error window
        exit(1);
    }
}
...
int main(int argc, const char* argv[]) {
    ...
    if (argc != 2) {
        printf("Syntax: SigintSender.exe <pid>");
        return 1;
    }
    int pid = atoi(argv[1]);
    sendCtrlC(pid);
    printf("Successfully sent CTRL+C to pid %d", pid);
}
